...
const rebootDevice = () => {
    ...
    postData('/restart', {key: "123"})
        .then(data => {
            if(data.status == 200){
                messageBox("ok")
                alert("The device has been reset")
            }else{
                messageBox("error")
            }
        });
...
const autoStartTask = async () => {
    ...
    console.log(jsons)
    if(jsons.brightness == -1){
       inputSriderbrightness.disabled = true;
       inputBrightnessContainer.classList.add('disabel')
       inputSelectbrightness[0].checked = true;
    }else{
        inputSriderbrightness.value = jsons.brightness
        rageUpdate(1)
        setValue()
    }
    if(jsons.nightMode){
